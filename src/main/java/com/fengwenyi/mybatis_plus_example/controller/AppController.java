...
public class AppController {
    ...
    @Autowired
    private CoinTestService coinTestService;
    // 查询所有城市
    ...
    // 查询所有城市
    @GetMapping("/queryCoinAll")
    public ResultModel queryCoinAll(CoinTest coinTest) throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {

        QueryWrapper query =  new QueryWrapper<CoinTest>();

        //query.eq("name",name);

        CoinTest coinTest1 = coinTestService.getOne(query);
        //CoinTest coinTest = coinTestService.queryCityAll(name);
        return ResultHelper.success("Success", coinTest1);
    }
